"""Helper script: regenerate src/cli/generate.py cleanly."""
import pathlib
...
def generate_explainer(
    ...
    title = f"Como funciona: {inst.name if inst else institution}"
    body_parts = []
...
def generate_profile(
    ...
    title = f"Perfil: {fig.name if fig else figure}"
    draft = ContentDraft(
...
pathlib.Path("src/cli/generate.py").write_text(CONTENT, encoding="utf-8")
print("generate.py written OK, lines:", CONTENT.count("\\n"))
